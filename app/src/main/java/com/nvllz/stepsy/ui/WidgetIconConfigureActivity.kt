...
class WidgetIconConfigureActivity : Activity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val previewContainer     = findViewById<FrameLayout>(R.id.preview_widget_icon_container)
        ...
        saveButton.setOnClickListener {
            ...
            WidgetIconProvider.updateWidget(this, appWidgetId, steps)
    ...
    private fun updatePreviewColor(useDynamicColors: Boolean, opacity: Float) {
        ...
        val colorRes = if (useDynamicColors && android.os.Build.VERSION.SDK_INT >= 31) {
        ...
        findViewById<FrameLayout>(R.id.preview_widget_icon_container).background = drawable
    ...
    private fun updatePreviewBackgroundColor(inverse: Boolean) {
        ...
        findViewById<LinearLayout?>(R.id.outer_widget_icon_container)?.setBackgroundColor(color)
    }

    private fun applyWidgetColors(useDynamicColors: Boolean) {
        val ctx = themedContext(currentThemeMode())

        val primaryRes = if (useDynamicColors && android.os.Build.VERSION.SDK_INT >= 31) {
            R.color.widgetPrimary
        } else {
            R.color.widgetPrimary_default
        }

        val secondaryRes = if (useDynamicColors && android.os.Build.VERSION.SDK_INT >= 31) {
        ...
        findViewById<ImageView?>(R.id.preview_widget_icon_img)?.apply {
            ContextCompat.getDrawable(context, R.drawable.widget_icon)?.mutate()?.let {
                it.setTint(primaryColor)
                setImageDrawable(it)
            }
        }

        findViewById<TextView?>(R.id.preview_widget_icon_steps)?.setTextColor(primaryColor)
        findViewById<TextView?>(R.id.preview_widget_icon_distance)?.setTextColor(secondaryColor)
    }

    private fun applyTextSizeScale(scalePercent: Int) {
        val stepsText    = findViewById<TextView>(R.id.preview_widget_icon_steps)
        val distanceText = findViewById<TextView>(R.id.preview_widget_icon_distance)
        val factor = scalePercent / 100f
        stepsText.textSize    = 24f * factor
        distanceText.textSize = 14f * factor
    }
}